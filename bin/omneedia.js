...
try {
	...
	console.log(e);
	console.log('  - Installing libraries (1st time setup)');
	console.log('  It can take a LONG time... Please be patient!');
	var exec = require('child_process').exec;
	var script = "var exec = require('child_process').exec;exec('dir',function(e,d,er){console.log(d);});";
	script = '"'+script+'"';
	var cmd2 = 'node -e '+script;
	var cd = exec('cd bin & npm install',function(err,stdout,strerr) {
		console.log(err);
		console.log(strerr);
		console.log(stdout);
		console.log('  Done.');
...
function build_native()
{
	...
	shelljs.exec("cordova create native");
	// write config.xml
	var config=[];
	...
	config.push('    <name>'+Manifest.title+'</name>');
...
function fieldz(db,res,response,data) {
	if (fs.existsSync(PROJECT_WEB+path.sep+"Contents"+path.sep+"Settings"+path.sep+response.db+".scheme"))  {
		var json=fs.readFileSync(PROJECT_WEB+path.sep+"Contents"+path.sep+"Settings"+path.sep+response.db+".scheme");
...
*/	
	...
	if (argv.indexOf('start')>-1)
	{
		...
		*/
		...
		if (fs.existsSync(__dirname+path.sep+".."+path.sep+"cdn"+path.sep+"cdn"))
		app.use('/cdn',express.static(__dirname+path.sep+".."+path.sep+"cdn"+path.sep+"cdn"));

		if (fs.existsSync(__dirname+path.sep+".."+path.sep+".."+path.sep+"cdn"+path.sep+"cdn"))
		app.use('/cdn',express.static(__dirname+path.sep+".."+path.sep+".."+path.sep+"cdn"+path.sep+"cdn"));
		...
		*/
		...
		if (fs.existsSync(PROJECT_SYSTEM+path.sep+"app.js")) {
			...
			_App.upload={
				...
				toFile: function(filename,newfilename) {
